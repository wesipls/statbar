...
int main() {
  ...
  if ((status = rsmi_init(RSMI_INIT_FLAG_ALL_GPUS)) != RSMI_STATUS_SUCCESS) {
    ...
    if ((status = rsmi_dev_busy_percent_get(gpu_id, &gpu_busy_percent)) != RSMI_STATUS_SUCCESS) {
    ...
    uint64_t temp;
    if ((status = rsmi_dev_temp_metric_get(gpu_id, 0, RSMI_TEMP_CURRENT, (int64_t *)&temp)) != RSMI_STATUS_SUCCESS) {
